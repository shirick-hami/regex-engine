...
/**
 * Comprehensive tests for the NFAMatcher.
 * Tests Thompson's NFA simulation algorithm for regex matching.
 */
@DisplayName("NFAMatcher Tests")
class NFAMatcherTest {
    ...
    private MatchResult matchFull(String pattern, String input) {
        ...
        NFAMatcher matcher = new NFAMatcher(ast, DEFAULT_TIMEOUT);
    ...
    class WhitespaceTests {
        ...
        void testMatchWhitespace(String ws) {
            ...
            assertTrue(result.isMatched(), "Should match whitespace: " + (int) ws.charAt(0));
        }
    ...
    class FindAllTests {
        ...
        @Test
        @DisplayName("Overlapping matches - non-overlapping returned")
        void testNonOverlapping() {
            MatchResult result = findAll("aa", "aaaa");
            assertTrue(result.isMatched());
            assertEquals(2, result.getAllMatches().size());
        }
    }
    ...
    @Nested
    @DisplayName("NFA-Specific Behavior Tests")
    class NFABehaviorTests {
        
        @Test
        @DisplayName("NFA handles pathological patterns efficiently")
        void testPathologicalPattern() {
            // This pattern would cause exponential backtracking
            // but NFA handles it in linear time
        ...
        @Test
        @DisplayName("NFA tracks state transitions")
        void testStateTransitions() {
            MatchResult result = matchFull("a*b", "aaab");
            assertTrue(result.isMatched());
            assertTrue(result.getBacktrackCount() > 0, "Should track state transitions");
        }
        ...
        void testManyBranches() {
            ...
            for (char c = 'a'; c <= 'z'; c++) {
                assertTrue(matchFull(pattern, String.valueOf(c)).isMatched());
            }
        }
    }
    ...
    @Nested
    @DisplayName("Epsilon Closure Tests")
    class EpsilonClosureTests {
        
        @Test
        @DisplayName("Multiple epsilon transitions")
        void testMultipleEpsilon() {
            // Pattern with many epsilon transitions (empty alternatives)
            MatchResult result = matchFull("(a|b|c)*", "abc");
            assertTrue(result.isMatched());
        }
        
        @Test
        @DisplayName("Nested optional groups")
        ...
        @Test
        @DisplayName("Alternation with empty option")
        void testEmptyAlternative() {
            // Both should match - 'a' option or 'ab' option
            MatchResult result1 = matchFull("a(b|)", "a");
            assertTrue(result1.isMatched());
            
            MatchResult result2 = matchFull("a(b|)", "ab");
            assertTrue(result2.isMatched());
        }
    }
}
